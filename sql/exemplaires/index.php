...
// Sous-requete : nombre d'emprunts par exemplaire via la table Emprunter.
// Disponibilite : aucun emprunt en cours (retour non enregistre).
$exemplaires = $pdo->query("
    ...
    (SELECT COUNT(*) FROM Emprunter em
        ...
        WHERE em.Id_Exemplaire = ex.Id_Exemplaire AND e.dateretour IS NULL) AS nb_emprunts_en_cours
    FROM Exemplaire ex
        ...
        <table>
            ...
            <tbody>
                ...
                <?php foreach ($exemplaires as $ex): ?>
                    <tr>
                        <!-- Code unique = Dewey + rang d'arrivee -->
                        <td><strong><?php echo htmlspecialchars($ex['code_exemplaire'] ?? ('#' . $ex['Id_Exemplaire'])); ?></strong></td>
                        ...
                        <!-- Disponible si aucun emprunt en cours -->
                        <td>
                            <?php if ($ex['nb_emprunts_en_cours'] == 0): ?>
